...
<button type="submit">\u{414}\u{43E}\u{434}\u{430}\u{442}\u{438} \u{43A}\u{43E}\u{43C}\u{435}\u{43D}\u{442}\u{430}\u{440}</button>
    ...
    `).join("")})}catch(t){console.log(t)}},o=async t=>{let e={method:"POST",body:JSON.stringify(t),headers:{"Content-Type":"application/json; charset=UTF-8"}};try{return await fetch("http://localhost:3000/posts",e).then(t=>t.json())}catch(t){console.log(t)}},n=async(t,e)=>{let o={method:"PUT",body:JSON.stringify(e),headers:{"Content-Type":"application/json; charset=UTF-8"}};try{return await fetch(`http://localhost:3000/posts/${t}`,o).then(t=>t.json())}catch(t){console.log(t)}};document.querySelector("#load").addEventListener("click",async()=>await e()),document.querySelector("#postsContainer").addEventListener("click",async o=>{if("Видалити"===o.target.textContent){let n=o.target.parentElement;console.log(n.id),await t(n.id),await e()}console.log(o.target.parentElement)}),document.querySelector("#createPostForm").addEventListener("submit",async t=>{t.preventDefault();let n=t.target.elements.nameCollect.value,u=t.target.elements.discriptionCollect.value;await o({title:n,text:u,comments:[]}),await e()});let u="";document.querySelector("#postsContainer").addEventListener("click",async t=>{"Редагувати"===t.target.textContent&&(document.querySelector(".edit-backdrop").style.visibility="visible",document.querySelector(".edit-backdrop").style.display="flex",u=t.target.parentElement)}),document.querySelector(".form-edit").addEventListener("submit",async t=>{t.preventDefault();let o=t.target.elements.nameEditCollect.value,a=t.target.elements.discriptionEditCollect.value;document.querySelector(".edit-backdrop").style.visibility="hidden",document.querySelector(".edit-backdrop").style.display="none",await n(u.id,{title:o,text:a,comments:[]}),await e()}),document.querySelector(".createCommentForm").addEventListener("submit",t=>{t.preventDefault,console.log(t.target.elements.commentCollect.value)});
